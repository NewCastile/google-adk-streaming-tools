...
const monitorStreamButton = document.getElementById("monitorStreamButton");
const monitorPriceButton = document.getElementById("monitorPriceButton");
/** string */
...
function initializeStreamingTools() {
  console.log(monitorPriceButton, monitorStreamButton);

  monitorPriceButton.addEventListener("click", (e) => {
    e.preventDefault();
    sendMessage({ mime_type: "text/plain", data: monitorPricePrompt });
  });

  monitorStreamButton.addEventListener("click", (e) => {
    e.preventDefault();
    sendMessage({ mime_type: "text/plain", data: monitorStreamPrompt });
  });
}
...
const screenButton = document.getElementById("startScreenShareButton");
  ...
  if (mediaHandler.isScreenActive) {
    ...
    if (success) {
      mediaHandler.startFrameCapture((base64Image) => {
        ...
        console.log("[CLIENT TO AGENT] sent %s bytes", base64Image.length);
      });
    }
  }
});
